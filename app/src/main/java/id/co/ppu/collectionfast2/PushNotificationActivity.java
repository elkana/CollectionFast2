...
public abstract class PushNotificationActivity extends SyncActivity implements FragmentChatActiveContacts.OnChatContactsListener
        , FragmentChatWith.OnChatWithListener
        , FragmentChatAllContacts.OnContactSelectedListener {
    private static final String TAG = "PushNtfyActivity";
...
//    Timer timerQueueMessage = new Timer();
    ...
    private Runnable runnableChatStatus = new Runnable() {
        ...
        public void run() {
            ...
            NetUtil.chatLogOn(PushNotificationActivity.this, getCurrentUserId(), new OnSuccessError() {
                @Override
                public void onSuccess(String msg) {
                    //yg mana offline ?
                    NetUtil.chatUpdateContacts(PushNotificationActivity.this, getCurrentUserId(), new OnGetChatContactListener() {
                        @Override
                ...
                public void onSkip() {
                    //yg mana offline ?
                    NetUtil.chatUpdateContacts(PushNotificationActivity.this, getCurrentUserId(), null);
                }
            });
    ...
    private TimerTask timerTaskQueueMessage = new TimerTask() {
        @Override
        public void run() {
            if (Utility.isScreenOff(PushNotificationActivity.this) || !NetUtil.isConnected(PushNotificationActivity.this))
                return;
...
//                            realm.copyToRealmOrUpdate(anyTransmittingData);
                ...
                if (pendings.size() > 0) {
                    ...
                    try {
                        ...
                        e.printStackTrace();
                        Utility.throwableHandler(PushNotificationActivity.this, e, false);
                    }
...
//                    handleNotification(intent);
                    ...
                    Log.e(TAG, "selectedMenu:" + mSelectedNavMenuIndex + "\nnav_chats:" + R.id.nav_chats + "\nnav_news: " + R.id.nav_news);
                    ...
                    if (NewsUtil.isTitleIsNews(intent.getStringExtra("title"))) {
                        ...
                        String excerpt = intent.getStringExtra("body");
                        String key_uid = intent.getStringExtra(NewsUtil.KEY_UID);
                        String key_from = intent.getStringExtra(NewsUtil.KEY_FROM);
                        String key_to = intent.getStringExtra(NewsUtil.KEY_TO);
                        String key_article = intent.getStringExtra(NewsUtil.KEY_ARTICLE);
                        String key_msgType = intent.getStringExtra(NewsUtil.KEY_MSG_TYPE);
                        String key_timestamp = intent.getStringExtra(NewsUtil.KEY_TIMESTAMP);

                        Realm r = Realm.getDefaultInstance();

                        r.beginTransaction();
                        try {
                            // spy ga duplicate, uid di generate di pusat
                            TrnNews first = r.where(TrnNews.class).equalTo("uid", key_uid).findFirst();
                            ...
                            first.setMessage(key_article);
                            first.setMessageType(key_msgType == null ? NewsUtil.MESSAGE_TYPE_TEXT : key_msgType);
                            first.setMessageStatus(NewsUtil.MESSAGE_STATUS_UNOPENED_OR_FIRSTTIME);
                            first.setExcerpt(excerpt);
                            first.setTitle(title);
                            first.setSender(key_from == null ? "HQ" : key_from);
                            first.setTo(key_to == null ? "ALL" : key_to);
                            first.setCreatedTimestamp(TextUtils.isEmpty(key_timestamp) ? new Date() : Utility.convertStringToDate(key_timestamp, NewsUtil.TIMESTAMP_PATTERN));

                            r.copyToRealmOrUpdate(first);
...
//                    Utility.createAndShowProgressDialog(MainChatActivity.this, "No Contacts found", "You have empty List.\nPlease try again.");
                ...
                if (resp.getError() != null) {
                    Utility.showDialog(PushNotificationActivity.this, "Error (" + resp.getError().getErrorCode() + ")", resp.getError().getErrorDesc());
                    return;
                }
...
//                    ((FragmentChatWith) frag).listAdapter.notifyDataSetChanged();
    ...
    protected void changeFabIcon(int resId) {
        fab.setImageDrawable(AppCompatDrawableManager.get().getDrawable(PushNotificationActivity.this, R.drawable.ic_send_black_24dp));
    }
}
